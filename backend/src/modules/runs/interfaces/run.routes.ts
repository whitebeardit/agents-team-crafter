...
export async function registerRunRoutes(app: FastifyInstance, d: IAppDeps) {
  const tenant = [d.authenticate, d.requireTenant];

  app.get('/runs', { preHandler: tenant }, async (req, reply) => {
    const ws = req.workspaceId!;
    const q = listQuerySchema.parse(req.query);
    const runs = await d.runRepo.listRuns(ws, { teamId: q.teamId, limit: q.limit });
  ...
  app.get('/runs/:runId', { preHandler: tenant }, async (req, reply) => {
    ...
    const run = await d.runRepo.findRun(ws, runId);
    if (!run) throw new AppError('NOT_FOUND', 'Run nao encontrada', 404);
    const steps = await d.runRepo.listSteps(ws, runId);
  ...
  app.get('/runs/:runId/events', { preHandler: tenant }, async (req, reply) => {
    ...
    const run = await d.runRepo.findRun(ws, runId);
    if (!run) throw new AppError('NOT_FOUND', 'Run nao encontrada', 404);
    const events = await d.runRepo.listEvents(ws, runId);
  ...
  app.get('/teams/:id/runs', { preHandler: tenant }, async (req, reply) => {
    ...
    const runs = await d.runRepo.listRuns(ws, { teamId, limit: 30 });
    return reply.send(successEnvelope(runs));
  });
}
