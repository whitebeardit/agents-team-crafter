...
- requiredPacks: lista de identificadores de packs de negocio sugeridos quando o problema claramente exigir capabilities de dominio; use APENAS estes valores (strings exatas): ${PLANNER_PACK_IDS_PROMPT}. Use [] se nao aplicavel. Exemplos: atendimento clinico / prontuario (clinic_ops preferencial; care/clinical como blocos universais), pacotes e sessoes (clinic_ops preferencial para clinica; packages_encounters como universal), contas a receber (finance), cadastro (crm), agenda (scheduling).
...
- **Clinica GOLD / workflows humanos:** se o briefing mencionar clinica, saude, psicologia, paciente, sessao, pacote, prontuario, atendimento clinico, agenda clinica ou cobranca de sessao, prefira o pack "clinic_ops" e actions "clinic_*" como camada primaria de produto. Exemplos: "clinic_create_patient", "clinic_schedule_session_by_phone", "clinic_register_attendance_by_phone_and_time", "clinic_get_patient_full_snapshot", "clinic_create_receivable_for_session". Use "crm_*", "schedule_*", "finance_*", "clinical_*", "package_*" e "attendance_*" como primitivas universais ou fallback/blocos internos quando nao houver workflow "clinic_*" adequado. Especialistas clinicos devem ter requiredPackIds com "clinic_ops" ou requiredBusinessActionIds "clinic_*" coerentes com seu papel.
...
export function buildTeamPlannerUserMessage(
  ...
  return [
    ...
    'Se o briefing for clinico/saude/psicologia/paciente/sessao/pacote/prontuario/cobranca clinica, priorize requiredPacks ["clinic_ops"] e actions clinic_*; use crm_*/schedule_*/finance_*/clinical_*/package_* so como fallback ou bloco universal.',
