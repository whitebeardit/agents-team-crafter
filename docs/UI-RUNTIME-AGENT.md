...
| **Ferramentas** (catálogo) | `capabilities.tools` | IDs em [`available-tools`](../backend/src/modules/agents/domain/available-tools.ts): cada um vira **function tool** do SDK. Com integração por workspace (ver abaixo), `calendar_access` e `image_generation` podem executar chamadas reais; sem integração, resposta de stub/indisponível. **CRM** de domínio não usa catálogo HTTP: use o pack `crm` / `internal_action` (`crm_*`) sobre o MongoDB. |
...
| **Integrações (Settings → Integrações)** | `Workspace` secrets cifrados | `toolCalendar` e chave OpenAI (BYOK ou ambiente) alimentam os executores built-in das tools de catálogo acima. CRM externo, se necessário no futuro, documenta-se como `http_webhook` ou MCP — não há `toolCrm` no catálogo. |
