...
- **[Loops 91–95](#loops-88-operacao-real-ux-troubleshooting)** — **candidatos (backlog recomendado após o 90)** — continuação da macro-onda de **operação real**: console conversacional, CTAs de resolução, runs legíveis, templates operacionais, polimento responsivo; detalhe na secção dedicada.
- **[Loops 96+](#loops-88-mais-verticais-de-negócio-por-pack)** — **planeados** — um slice Ralph por vertical (ou grupo justificado) alinhado a `packId` (`care`, `finance`, `scheduling`, …), **após** a onda operacional 91–95 ou em paralelo conforme priorização; numerar ao abrir; detalhe na secção dedicada.
...
**Próximo recorte recomendado (candidato numerado):** **[Loop 91](#loop-91--console-conversacional-com-sessões-timeline-e-contexto-reutilizável)** (console conversacional com sessões e timeline), seguido da macro-onda **[Loops 91–95](#loops-88-operacao-real-ux-troubleshooting)**; verticais por `packId` **[96+](#loops-88-mais-verticais-de-negócio-por-pack)** quando aplicável.
...
## Loops 88+ — operação real dos times, UX de operação e troubleshooting *(em curso; Loops **88–90** fechados no [ledger](agents-team-crafter-plano-evolucao_IMPLEMENTADO.md); continuação **91–95**)*

**Pré-requisitos:** **[Loop 87](#loop-87-especialistas-operacionais-schemas-reais-coleta-de-dados-faltantes-e-contexto-conversacional)** até **[Loop 90](#loop-90--cockpit-operacional-do-team)** estão **entregues** no [ledger](agents-team-crafter-plano-evolucao_IMPLEMENTADO.md). Não antecipar entregas nem colapsar vários temas num único PR.

Os **Loops 91–95** abaixo são a **continuação recomendada** após o 90: cada um deve permanecer **pequeno**, com **gate** (`./scripts/ralph-loop-gate.sh` e frontend quando aplicável) e **documentação** no [ledger](agents-team-crafter-plano-evolucao_IMPLEMENTADO.md). **Não** prometer mega-entregas; **evitar** misturar console de debug, runs e CTAs de resolução no mesmo slice quando o recorte deixar de ser coerente.

<a id="loop-88--preflight-operacional-do-team--readiness-do-runtime"></a>

### Loop 88 — Preflight operacional do team / readiness do runtime *(fechado — ver [ledger Loop 88](agents-team-crafter-plano-evolucao_IMPLEMENTADO.md#loop-88-fechado))*
...
### Loop 90 — Cockpit operacional do team *(fechado — ver [ledger Loop 90](agents-team-crafter-plano-evolucao_IMPLEMENTADO.md#loop-90-fechado))*
...
<a id="loop-91--console-conversacional-com-sessões-timeline-e-contexto-reutilizável"></a>
...
### Ordem recomendada após o Loop 90 (candidatos 91–95)

1. Loop 91 — console conversacional  
2. Loop 92 — resolver pendências  
3. Loop 93 — runs / replay / troubleshooting  
4. Loop 94 — templates operacionais  
5. Loop 95 — polimento responsivo final  
...
40. **Loops 91–95** — macro-onda de **operação real** (console conversacional, CTAs, runs, templates, polimento) *(candidatos / backlog recomendado após o 90; ver [Loops 88+ — operação](#loops-88-operacao-real-ux-troubleshooting))*.
...
- **Loop 87:** **runtime dos especialistas** — schemas estritos, `internal_action` com contrato, CRM + slot-filling, conversa de teste com histórico *(fechado; ver [Loop 87](#loop-87-especialistas-operacionais-schemas-reais-coleta-de-dados-faltantes-e-contexto-conversacional))*
...
- **Loops 91–95:** **operação real, UX e troubleshooting** — debug narrativo, runs legíveis, CTAs, etc. *(candidatos; ver [Loops 88+ — operação](#loops-88-operacao-real-ux-troubleshooting))*
...
**Fase actual (pós Loops 77–90):** a onda **team planner + AI Builder** (Loops **82–86**) está **fechada**; **87–90** fecharam especialistas, readiness, AI Builder em camadas e **cockpit** na ficha do time. **Próximo candidato numerado na macro-onda de operação:** [Loop 91](#loop-91--console-conversacional-com-sessões-timeline-e-contexto-reutilizável) e [Loops 91–95](#loops-88-operacao-real-ux-troubleshooting). **Paralelamente:** [14.8](#148-riscos-e-decisões-em-aberto) (billing, 2FA, self-service).

A fase mais recente consolidou **prontidão**, **modo simples no AI Builder** e **cockpit operacional** por time; o próximo salto continua a **operação confiável** na UI (debug narrativo, runs, CTAs, …). **Backlog recomendado (candidatos):** sequência **[91–95](#loops-88-operacao-real-ux-troubleshooting)**; verticais por domínio **[96+](#loops-88-mais-verticais-de-negócio-por-pack)** quando a prioridade o justificar.
...
2. **Vertical por domínio** — repetir o mesmo rigor **pack a pack** (ações, estados de negócio, listagens com defaults sensatos, testes de conversa). O Loop 87 inclui o **piloto CRM**; finanças, care e restantes entram como **Loops 96+** — ver secção [Loops 96+ — verticais por pack](#loops-88-mais-verticais-de-negócio-por-pack) (candidatos, critérios de priorização, tamanho do slice). **Não** confundir com a macro-onda de **operação / UX / troubleshooting** **[91–95](#loops-88-operacao-real-ux-troubleshooting)** (debug narrativo, runs, etc.; **readiness** no [Loop 88](#loop-88--preflight-operacional-do-team--readiness-do-runtime); **cockpit** no [Loop 90](#loop-90--cockpit-operacional-do-team)).
